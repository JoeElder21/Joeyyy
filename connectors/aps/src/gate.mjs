...
// APS SDK modules are loaded lazily (inside functions) so that the
// credential and argument checks in main() can exit with code 2 before
// any import error causes Node to exit with code 1.

const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const DEFAULT_MODEL = join(ROOT, "testdata", "aps_test_model.dxf");

async function apsScopes() {
  const { Scopes } = await import("@aps_sdk/authentication");
  const READ_SCOPES = [Scopes.DataRead, Scopes.ViewablesRead];
  const STEP4_SCOPES = [...READ_SCOPES, Scopes.DataWrite, Scopes.DataCreate, Scopes.BucketCreate, Scopes.BucketRead, Scopes.BucketDelete];
  return { READ_SCOPES, STEP4_SCOPES };
}
...
async function step2() {
  const { READ_SCOPES } = await apsScopes();
...
async function step4() {
  const { STEP4_SCOPES, READ_SCOPES } = await apsScopes();
  ...
  const { OssClient } = await import("@aps_sdk/oss");
...
async function cleanup() {
  ...
  const { STEP4_SCOPES } = await apsScopes();
  const accessToken = await token(STEP4_SCOPES);
