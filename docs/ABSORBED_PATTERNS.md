...
### Typed sufficiency verdict with the stop decision made outside the evaluator — from google/adk-samples deep-search

Give every iterative evidence pass an explicit sufficiency verdict, and never let the agent that produced the work decide when to stop. After each gathering pass a separate reviewer returns exactly one typed verdict: pass or fail, a written reason, and on fail the specific follow-up questions needed to close each named gap. A deterministic check outside both agents reads that verdict and ends the loop only on pass; the reviewer may not end the loop, delegate, or move control anywhere except back with its verdict. Cap the loop at a fixed iteration count declared in the delegation before the first pass. When the cap is reached without a pass, return status partial with the unmet gaps as blockers — never compose a completed artifact on an exhausted loop, because a capped-out loop is an unfinished job, not a passing one. Record every verdict, its reason, and its iteration number so the weekly audit can see how many passes each mission type actually needs and tighten the cap on evidence rather than guess. *Candidate merge: `<operating_method>` of `.codex/agents/apex_intelligence_forge.toml`.*

### Declared retrieval target per bucket with a reformulation ladder — from google/adk-samples academic-research

State how much evidence is enough before searching, bucket by bucket, and log what was tried when you fall short. The delegation names a minimum retrieval target for each bucket the mission must cover — per year, per agency, per project, per source type. When a bucket sits under its target, work an enumerated reformulation ladder instead of repeating the same query: change the phrasing of the relationship being searched, switch identifiers (full title, partial title plus lead author, permit or plan number, DOI), scope the search to one specific repository or portal, and combine known names with the date window. Continue until the target is met or the ladder is exhausted, then verify each hit actually satisfies the bucket's criteria and discard duplicates and low-confidence matches before counting. Report the count found for each bucket against its target, and when a target is not met, list the reformulations attempted. An unmet target with a recorded attempt log is an acceptable partial; an unmet target reported as complete is a manufactured metric.

### Branch-chained task ordering — from google/adk-samples sdlc-task-planner

Express every ordering dependency between delegated tasks as a branch base rather than as a note. Give each task an explicit source branch and target branch: tasks that may run at the same time all take the same base, and a task that depends on another takes its predecessor's target branch as its source. Every chain must end by merging back into the trunk, and no task may target a branch that nothing later merges. Check this before dispatch — an ordering that cannot be drawn as an acyclic branch graph converging on the trunk is a planning defect, not something to resolve at run time. This makes an ordering violation impossible to execute rather than merely disallowed, and it extends the one-worktree, one-branch, one-agent rule from parallel work to sequenced work. Record the source and target branch in the ticket frontmatter and in the delegation packet so handoff validation can confirm the return landed on the branch the packet named.

### Task-size ceiling enforced before dispatch — from google/adk-samples sdlc-task-planner

Set a written size ceiling for a single delegated task and enforce it while planning, not while running. State the ceiling in units the work actually has — for repository work, a maximum number of files touched and a maximum diff size; for document work, a maximum number of target sections. When decomposition produces a task above the ceiling, split it into ordered smaller tasks before any packet is issued; never dispatch an oversized task and rely on the agent to contain itself. This is preventive scope control and it sits in front of child-task spawning, which handles only work discovered after a task has started. Start from a ceiling you can defend, then adjust it from observed run outcomes at the weekly audit, and never present the current ceiling as a measured result. *Candidate merge: delegation-packet section, `docs/AGENT_COMMUNITY_PROTOCOL.md`.*
...
### Claim-level verdicts on returns, from a closed five-value set, anchored to a span — from google/adk-samples llm-auditor

Do not accept a specialist return as prose to be judged whole. Before integration, decompose the return's findings into distinct claims, each written as a standalone statement, and record with every claim the exact span of the return it came from, the evidence record IDs it rests on, and exactly one verdict from a closed set: accurate, inaccurate, disputed, unsupported, or not-applicable. Use disputed when authoritative sources genuinely conflict, unsupported when no source could be found, and not-applicable when the claim is a judgment, a preference, or a stated assumption that was never meant to carry external evidence. A claim with no span, no evidence ID, or no verdict is not a finding; treat it as an unverified assertion and keep it out of the integrated result. Keep the artifact-record confidence label for what the producing specialist declares about its own sourcing — the verdict is the reviewer's ruling and belongs to a different actor. Score the weekly audit's accuracy column from verdict counts per agent instead of an impression, and log every inaccurate and unsupported verdict to the error ledger so repeat producers surface across weeks. *Candidate merge: typed findings in `schemas/handoff_packet.schema.json` and the specialist-return section of `templates/specialist-handoff.md`.*

### Verdict-bounded repair with a no-new-claims termination rule — from google/adk-samples llm-auditor

Give the repair step less authority than the audit step. Once a return carries claim-level verdicts, the permitted edit is fixed by the verdict: leave accurate and not-applicable claims untouched, correct an inaccurate claim to the reviewer's justification and nothing further, present the conflicting positions for a disputed claim rather than choosing one, and for an unsupported claim either drop it when it is not central or keep it explicitly labeled as unsupported. The repair may not introduce a new claim, a new source, or a new statement, and it must keep the return's structure and length close to the original; a repair that adds material has become new work and needs its own delegation. Enforce the no-new-claims rule with the tool surface rather than the instruction — the repairing step gets no search, retrieval, memory, or connector access, so it has nothing to add from. Because a repaired return then contains no unaudited material, one audit pass and one repair pass are sufficient; do not loop, and do not re-audit a repair whose only changes were licensed by a verdict. If a verdict cannot be satisfied inside these limits, return the packet to its specialist with the failing claims named, instead of editing around it or discarding a whole return over one bad claim.

### Read-coverage contract on every batch document read — from google/adk-samples high-volume-document-analyzer

Every batch document read declares its mode before it starts — exhaustive, meaning every document must be read, or sufficient-answer, meaning stop as soon as the answer is found — and carries a read-coverage ledger back with its result. The ledger lists one outcome per document: read, skipped with reason, or failed with reason. Report the count of documents actually read, never the count attempted; a batch where seven of forty PDFs failed to download produces a summary over thirty-three documents and says so in the same breath. Advance the intake cursor only after the batch result and its ledger are durably recorded — advancing before the extraction step means a failed batch is skipped silently and its documents are never read and never missed. Documents that exhaust their retry budget move to a dead-letter list at their own position, named in the ledger, instead of failing the batch around them. Never strip the termination signal from the answer: a sufficient-answer read that stopped at document nine must not be presented as having read the file. Reporting coverage the pipeline did not achieve is a manufactured metric and falls under the same hard rule as any other. *Candidate merge: APEX Intelligence Forge gate, `docs/SPECIALIST_ACCEPTANCE_TESTS.md`.*
...
### Correlation and brain keys on every telemetry record — from google/adk-samples agent-observability-bq (absorbed AND implemented 2026-07-24)

Give every telemetry record the keys needed to group it later: the mission it belongs to, the delegation it came from, the agent that produced it, that agent's brain, and the id of the record it descends from. Record the brain on the record rather than deriving it at read time from the roster — the roster changes, the history must not. Carry the identical key set on the span attributes and on the ledger entry so a span and its ledger entry can be matched to each other and both back to the mission. Without the parent key a flat log cannot be rebuilt into the tree of one mission's handoffs, and without the brain key the APEX and JEOS rows of the weekly scorecard have no evidence behind them. Keys carry identifiers only, never packet content. Treat a key missing from an older record as unknown, never as zero. Implemented in `scripts/observability.py` (`MissionTracer._keys`) and `scripts/agent_runtime.py` (ledger detail dicts).

### Weekly review reads the durable store, not the live process — from google/adk-samples agent-observability-bq (absorbed AND implemented 2026-07-24)

Write each span to the durable ledger as it closes, and make the weekly aggregation read that ledger over a stated date window instead of reading the exporter still held in memory. An aggregation that reads the live exporter can only see its own process, so a week of separate runs sums to nothing and the audit falls back to narrative — the exact failure the tracing was built to end. Keep the in-memory exporter for tests and the ledger for history, both fed from the same call sites, so how operations are traced does not change. Print the window alongside every count so a number can never be read as covering more than it does. When the window holds no records, report that as the result rather than reporting zeros. Implemented in `scripts/observability.py` (`weekly_review(since, until)`), with a test proving two separate tracer runs aggregate through one shared ledger.
