...
- **Signed-identity allowlist enforcement (2026-07-25)** — the `agents` field is now an actual authorization decision. `grant` takes `--agent`, refuses any identity absent from that mount's allowlist, and signs the identity into the HMAC payload; `authorize()` trusts only the signed value, so a grant holder cannot claim a different agent and tampering breaks the signature. Previously the field was inert, and an intermediate fix trusted a caller-supplied launch flag. Applies to every mount not declaring `agents = ["*"]` — `civil3d` included — and a test enforces that every agent-scoped mount also requires a grant.
- **terraform / azure** — registered, Agent-007-only, grant-gated; see `docs/TERRAFORM_AZURE_MCP_BUILDOUT.md`. Official HashiCorp and Microsoft servers, image/package pinned so the tool surface cannot change without a commit. Terraform registry lookup needs no credential; TFE workspace tools need `TFE_TOKEN`/`TFE_ADDRESS`, and Azure needs `az login` or the `AZURE_*` triple. Both refuse to launch without a Joe-signed one-time grant — verified fail-closed.
