...
- **Allowlist enforcement (2026-07-25)** — `scripts/trusted_launcher.py` now checks the launch identity against each mount's `agents` list and refuses anything absent from it, logging the rejected identity. Previously the field was inert, so `packet_only_no_direct_connectors` was only advisory at launch time. `launch` takes `--agent`, required for every mount that does not declare `agents = ["*"]` — `civil3d` included.
