...
| Test suite | 470 tests, 0 failures, 31 dependency-gated skips (36 without PyYAML) |
...
A complete, tested contract-and-enforcement layer with two runtime implementations on top of it (`runtime/` for stdlib-pure enforcement, `scripts/` for SDK integration), adapters written against ten major agent frameworks, one Node connector harness for Autodesk Platform Services, a 29-document architectural record, and 243 passing tests. What does *not* exist: a live deployment. No agent has been promoted past shadow, no connector has been credentialed, no memory backend has been selected, and no real mission has been run.
...
| Term | Meaning |
| --- | --- |
| **Agent 007** | Operating alias of `apex_chief_of_staff`. Sole cross-brain agent, sole holder of writer leases while specialists are in shadow, final integrator of every mission. Activated by the literal phrase *"Activate Agent 007"*, answered with exactly *"Agent 007 activated."* |
...
| `tests/` | 26 unittest modules, 470 tests. Dependency-gated tests skip cleanly, so the whole suite still runs stdlib-only — the privacy guard's parser tests are gated on PyYAML exactly as the guard itself degrades without it. | 27 |
...
**470 tests across 26 modules; 0 failures; 31 skipped** on Python 3.11 (36 skipped with PyYAML and reportlab absent). `tests/test_governance_docs.py` asserts these figures against a live run, so a stale count fails the suite rather than being published as evidence.
...
Joeyyy is a contract-first, enforcement-backed multi-agent governance system with two sealed data domains and eleven agents, of which one is active. Its distinguishing property is that it refuses to claim capability it cannot demonstrate — and it has 470 tests, a privacy scanner, a fail-closed packet validator and a denial-first launcher to keep that refusal honest.
