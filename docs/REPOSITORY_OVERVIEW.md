...
| Test suite | 486 tests, 0 failures, 31 dependency-gated skips (36 without PyYAML) |
...
| `tests/` | 26 unittest modules, 486 tests. Dependency-gated tests skip cleanly, so the whole suite still runs stdlib-only — the privacy guard's parser tests are gated on PyYAML exactly as the guard itself degrades without it. | 27 |
...
**486 tests across 26 modules; 0 failures; 31 skipped** on Python 3.11 (36 skipped with PyYAML and reportlab absent). `tests/test_governance_docs.py` asserts these figures against a live run, so a stale count fails the suite rather than being published as evidence.
...
Joeyyy is a contract-first, enforcement-backed multi-agent governance system with two sealed data domains and eleven agents, of which one is active. Its distinguishing property is that it refuses to claim capability it cannot demonstrate — and it has 486 tests, a privacy scanner, a fail-closed packet validator and a denial-first launcher to keep that refusal honest.
