...
Repository side: two concerns share this branch, and the file lists are the
seam between them under any merge method. The WSL layer itself is
...
repairs are the four `requirements/lock*.txt` re-resolutions and the
`osv-scanner.toml` triage updates, including the chromadb reassessment this
install path triggered. On the branch they are separate commits and each
concern reverts on its own; if the branch lands as one squashed commit,
reverting that commit reverts both concerns together, and restoring only one
means reverting the files of that concern's list.
