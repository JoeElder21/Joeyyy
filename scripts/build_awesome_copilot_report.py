...
def manifest_names() -> dict[str, set[str]]:
    ...
    found = re.findall(r"`([A-Za-z0-9._-]+(?:\.instructions\.md|\.agent\.md))`", "\n".join(rows))
    ...
    skills = re.findall(r"^\s*[-*] `([A-Za-z0-9][A-Za-z0-9._-]*)/`", text, re.MULTILINE)
    return {
...
def rendered_names() -> dict[str, set[str]]:
    ...
    """
    return {
        "instructions": {f"{name}.instructions.md" for name, _glob, _why in INSTRUCTION_ROWS},
...
def reconcile_with_manifest() -> None:
    ...
    if not claimed:
        raise SystemExit(
            "cannot read .github/AWESOME-COPILOT.md to reconcile "
    ...
    for kind, expected in claimed.items():
        ...
        if untracked:
            problems.append(f"{kind}: in manifest but not tracked: {', '.join(sorted(untracked))}")
        if unlisted:
            ...
            # than silent.
            first_party.append(
                f"{kind}: not from upstream, excluded from the "
                f"adopted counts: {', '.join(sorted(unlisted))}"
            )
        if unrendered:
            problems.append(
                f"{kind}: adopted but this report renders no row "
                f"for it: {', '.join(sorted(unrendered))}"
            )
        if extra_rows:
            problems.append(
                f"{kind}: this report renders a row for an asset "
...
BODY = S("b", "BodyText", fontSize=9.7, leading=13.6, textColor=INK, spaceAfter=7)
CELL = S("c", "BodyText", fontSize=8.3, leading=10.8, textColor=INK, spaceAfter=0)
CELLB = S(
    "cb",
    "BodyText",
    fontSize=8.3,
...
NOTE = S("n", "BodyText", fontSize=8.6, leading=12, textColor=MUTED, spaceAfter=6)
...
def rule(space_after=10):
    story.append(
        HRFlowable(width="100%", thickness=0.7, color=RULE, spaceBefore=3, spaceAfter=space_after)
    )
...
def hdr(*labels):
    return [Paragraph(f'<font color="white"><b>{x}</b></font>', CELL) for x in labels]


# ---------------------------------------------------------------- cover block
story.append(Paragraph("Awesome Copilot — Selection Report", TITLE))
story.append(
    Paragraph("Which customizations were adopted into <b>JoeElder21/Joeyyy</b>, and why", SUB)
)
story.append(Spacer(1, 9))

meta = [
    [
        Paragraph("Prepared for", CELL),
        Paragraph("Joe Elder", CELLB),
        Paragraph("Decision date", CELL),
        Paragraph("25 July 2026", CELLB),
    ],
    [
        Paragraph("Prepared by", CELL),
        Paragraph("Agent 007 / APEX Chief of Staff", CELLB),
        Paragraph("Pull request", CELL),
        Paragraph("#26", CELLB),
    ],
    [
        Paragraph("Upstream source", CELL),
        Paragraph("github/awesome-copilot", CELLB),
        Paragraph("Pinned commit", CELL),
        Paragraph(manifest_pin(), MONO),
    ],
    [
        Paragraph("Built", CELL),
        Paragraph(BUILD_DATE, CELLB),
        Paragraph("", CELL),
        Paragraph("", CELL),
    ],
]
story.append(
    tbl(meta, [0.95 * inch, 2.35 * inch, 0.85 * inch, 1.55 * inch], header=False, zebra=False)
)
...
PRIMARY_AVAILABLE = (
    sum(UPSTREAM_INVENTORY.get(k, 0) for k in ("instructions", "agents", "skills")) or None
)
...
for cls, avail, took in [
    (
        "Instructions",
        UPSTREAM_INVENTORY.get("instructions"),
        count_adopted("instructions", "instructions/*.instructions.md"),
    ),
    ("Agents", UPSTREAM_INVENTORY.get("agents"), count_adopted("agents", "agents/*.agent.md")),
    ("Skills", UPSTREAM_INVENTORY.get("skills"), count_adopted("skills", "skills/*/SKILL.md")),
]:
    inv.append(
        [
...
for cls in ("plugins", "extensions", "hooks", "workflows"):
    ...
    inv.append(
        [
            Paragraph(cls.title(), CELL),
            Paragraph(_cell(avail), CELL),
            Paragraph("0", CELL),
            Paragraph("—", CELL),
        ]
    )
...
story.append(
    tbl(
        inv,
        [2.0 * inch, 1.55 * inch, 1.05 * inch, 1.1 * inch],
        align=[
...
story.append(
    Paragraph(
        ...
        "machinery this repository does not need.",
        NOTE,
    )
)
...
for sig, val, why in [
    ...
    (
        "Markdown",
        f"{MARKDOWN_PRE_INSTALL} files"
        if MARKDOWN_PRE_INSTALL >= 0
    ...
    ("Test modules", "24 files", "A real gate exists, so anything adopted must pass it."),
    (
        "Domain",
    ...
    prof.append([Paragraph(f"<b>{sig}</b>", CELL), Paragraph(val, CELL), Paragraph(why, CELL)])
...
if MARKDOWN_NOW < 0:
    _md_delta = (
        "The current markdown total could not be measured here "
        "(git ls-files did not succeed), so no delta is reported."
    )
elif MARKDOWN_ADDED >= 0:
    _md_delta = (
        f"This branch adds {MARKDOWN_ADDED} markdown files, so the tree now carries {MARKDOWN_NOW}."
    )
else:
    _md_delta = (
        f"The tree now carries {MARKDOWN_NOW} markdown files; the "
...
story.append(
    Paragraph(
        ...
        f"{_cell(UPSTREAM_INVENTORY.get('instructions'))})",
        H1,
    )
)
story.append(
...
INSTRUCTION_ROWS = [
    ...
    ("markdown", "**/*.md", "CommonMark 0.31.2 discipline across a markdown-heavy contract tree."),
    ("security-and-owasp", "**", "OWASP Top 10 2025 plus AI/LLM-specific guidance."),
    (
...
for f, glob, why in INSTRUCTION_ROWS:
    ins.append([Paragraph(f, MONO), Paragraph(glob, MONO), Paragraph(why, CELL)])
...
story.append(
    Paragraph(
        ...
        f"{_cell(UPSTREAM_INVENTORY.get('agents'))})",
        H1,
    )
)
story.append(
...
for f, role, note in AGENT_ROWS:
    ag.append([Paragraph(f, MONO), Paragraph(role, CELL), Paragraph(note, CELL)])
...
story.append(
    Paragraph(
        ...
        "function and route through intake.",
        NOTE,
    )
)
...
story.append(
    Paragraph(
        ...
        f"{_cell(UPSTREAM_INVENTORY.get('skills'))})",
        H1,
    )
)
story.append(
    Paragraph(
        ...
        "upstream from inside a session.",
        BODY,
    )
)
...
SKILL_ROWS = [
    (
        ...
        "Compares local instructions against upstream; flags additions and outdated files.",
    ),
    (
        "suggest-awesome-github-copilot-agents",
        "Same for custom agents, avoiding duplicates already present.",
    ),
    ("suggest-awesome-github-copilot-skills", "Same for skills."),
]
...
story.append(
    Paragraph(
        (
            ...
            else "The great majority of the collection was not adopted."
        )
        + " The exclusions fall into a small number of reasons, each of which is a "
        "positive judgement rather than an oversight.",
        BODY,
    )
)
...
for g, r in [
    ...
    (
        ...
        "Add runtime machinery and scheduled automation beyond the requested scope.",
    ),
    (
...
story.append(
    Paragraph(
        ...
        "added to one of those same files is still caught.",
        BODY,
    )
)
...
pg.append(
    [
        Paragraph(
            "Directory-wide exemption — skip two patterns for everything "
        ...
        Paragraph(
            "A tracked <font face='Courier' size='7.5'>local.instructions.md"
...
pg.append(
    [
        Paragraph("Per-file, per-pattern relaxation — one file gives up one pattern", CELL),
        Paragraph('<font color="#8A5A00"><b>rejected</b></font>', CELL),
        Paragraph(
            "Still too coarse. Disabling a whole pattern for a whole file "
...
pg.append(
    [
        Paragraph(
            "<b>Exact-literal stripping</b> — seven pinned snippets across "
            "three named files, every pattern applied to the rest",
            CELL,
        ),
        Paragraph('<font color="#1B6B4A"><b>in force</b></font>', CELL),
        Paragraph(
            "Only those literal strings are invisible. "
            ...
            "designs are permanent regression tests.",
            CELL,
        ),
    ]
)
...
story.append(
    Paragraph(
        ...
        "document was generated. Nothing here is a remembered result.",
        BODY,
    )
)
...
story.append(
    Paragraph(
        ...
        "the one claim this report and that manifest both rest on.",
        BODY,
    )
)
rule(6)
story.append(
    Paragraph(
        ...
        "agents, which declare broad tool permissions.",
        NOTE,
    )
)
