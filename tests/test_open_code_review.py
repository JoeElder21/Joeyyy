...
class OpenCodeReviewIntegrationTests(unittest.TestCase):
    def test_project_rules_are_valid_and_keep_private_state_out(self) -> None:
        rules = json.loads((ROOT / ".opencodereview" / "rule.json").read_text(encoding="utf-8"))
    ...
    def test_install_and_runtime_wrappers_share_the_exact_pin(self) -> None:
        installer = (ROOT / "scripts" / "install_open_code_review.sh").read_text(encoding="utf-8")
        wrapper = (ROOT / "scripts" / "open_code_review.sh").read_text(encoding="utf-8")
    ...
    @unittest.skipUnless(
        os.environ.get("OCR_INTEGRATION_TEST") == "1", "opt-in installed CLI check"
    )
