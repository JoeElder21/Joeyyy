...
class IdentityTests(unittest.TestCase):
    def test_equity_canonical_form(self):
        asset = identity.equity("nvda", "xnas")
        self.assertEqual(asset.canonical, "eq:XNAS:NVDA")
        self.assertEqual(identity.validate(asset), [])

    def test_evm_contract_is_lower_cased_and_base58_is_not(self):
        evm = identity.crypto("TKN", "pulsechain", "0xABCDEFabcdef0123456789ABCDEFabcdef012345")
        self.assertEqual(evm.key, "0xabcdefabcdef0123456789abcdefabcdef012345")
    ...
    def test_validation_catches_bad_shapes(self):
        ...
        self.assertTrue(identity.validate(identity.AssetId("option", "UPST", "XCBO", "not-occ")))
    ...
    def test_parse_round_trips_and_rejects_junk(self):
        ...
        self.assertEqual(identity.parse("eq:XNYS:VLO").symbol, "VLO")
        with self.assertRaises(ValueError):
            identity.parse("NVDA")
        with self.assertRaises(ValueError):
            identity.parse("zz:XNAS:NVDA")
